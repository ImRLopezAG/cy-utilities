...
export class CyPOMNested<T extends Record<string, Record<string, string>> = {}> {
  #elements: T;
  private constructor(elements: T) {
    this.#elements = elements;
  }

  static create<T extends Record<string, Record<string, string>>>(elements: T): CyPOMNested<T> {
    if (!elements) {
      throw new Error('The elements must be an object');
    }
  ...
  getElement = (parent: KeyOf<T>, element: NestedKeys<T>): Cypress.Chainable<JQuery<HTMLElement>> =>
...
declare global {
  ...
  export type NestedKeys<T extends Record<string, Record<string, any>>> = KeyOf<T[KeyOf<T>]>
  namespace Cypress {
    interface Chainable {
      /**
